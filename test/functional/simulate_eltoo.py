...
class CloseTx(CTransaction):
    ...
    def __init__(self, payment_channel, setup_tx):
        ...
        if self.payment_channel.settled_refund_amount > self.payment_channel.settled_payment_amount:
            self.payment_channel.settled_payment_amount -= max(FEE_AMOUNT/2, self.payment_channel.settled_payment_amount)
            self.payment_channel.settled_refund_amount -= CHANNEL_AMOUNT - self.payment_channel.settled_payment_amount - FEE_AMOUNT
        else:
            self.payment_channel.settled_payment_amount -= max(FEE_AMOUNT/2, self.payment_channel.settled_refund_amount)
            self.payment_channel.settled_refund_amount -= CHANNEL_AMOUNT - self.payment_channel.settled_refund_amount - FEE_AMOUNT
        ...
        #   add refund output
        self.vout = [ CTxOut(self.payment_channel.settled_refund_amount, script_pkh) ]
        ...
        #   add refund output
        self.vout = [ CTxOut(self.payment_channel.settled_payment_amount, script_pkh) ]
...
class SimulateL2Tests(BitcoinTestFramework):
    ...
    def commit(self, tx, error_code=None, error_message=None):
        ...
        tx_hex = bytes_to_hex_str(tx.serialize_with_witness())
    ...
    def test1(self, payer_sweeps_utxo):
        ...
        if payer_sweeps_utxo:
            ...
            self.log.debug("btcdeb --tx=%s --txin=%s\n", bytes_to_hex_str(redeem_tx.serialize_with_witness()), bytes_to_hex_str(settle2_tx.serialize_with_witness()))
